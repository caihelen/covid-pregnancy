...
all_genes <- read.csv("/home/hsc26/pi_pattabiraman/project/covid-bulk/brain_degs/group_age_condP5_sars-group_age_condP5_pbs-FULL.csv")
logFC_sig <- 0.5
adjP_sig <- 0.05 
...
downreg_results$Description <- gsub("covid", "COVID", downreg_results$Description)
...
## Make the barplot of interesting pathways for P5 (upregulated pathways)


```{r, eval=T, echo=F, message=F}
upreg_results <- read.csv("brain-P5-upregulated-logFC-05.csv")
upreg_results$Description <- gsub("covid", "COVID", upreg_results$Description)
upreg_results$Description <- gsub("Tgf", "TGF", upreg_results$Description)
...
ggplot(plot_df, 
  ...
  coord_flip() +
  labs(title = "Upreg. Pathways in SARS-CoV-2 (P5)",
...
ggsave("brain-P5-upreg-pathways-barplot-top15-mono.pdf",
       height = 10.5,
       width = 15, 
       units="cm")
...
## Make the barplot of interesting pathways for P5 (downregulated pathways)

```{r, eval=F, echo=F, message=F}
downreg_results <- read.csv("brain-P5-downregulated-logFC-05.csv")
downreg_results$Description <- gsub("covid", "COVID", downreg_results$Description)
# 218 results here

metabolism_results <- c(198, 201, 210, 63, 104, 91, 36, 118, 134, 194)
...
immuno_results <- c(1, 189, 196, 14, 190, 192, 15, 18, 22, 17, 19, 20, 29, 187)
immuno_results <- downreg_results[immuno_results,]
immuno_results <- immuno_results %>% arrange(p_adj * -1)
immuno_results$Category <- "Immunologic"


plot_df <- rbind(metabolism_results, immuno_results)
plot_df$Description <- factor(plot_df$Description, levels = plot_df$Description)
######### Create the horizontal barplots
...
ggplot(plot_df, 
  ...
  coord_flip() +
  labs(title = "Downreg. Pathways in SARS-CoV-2 (P5)",
       x="", y =bquote(-log[10]~"P-adj")) +
  geom_hline(yintercept=-1*log10(0.05), linetype="dashed", color="black", size=0.5)+
  theme_classic(base_size=12) +
...
ggsave("brain-P5-downreg-pathways-barplot-split.pdf",
       height = 10.5,
       width = 15, 
       units="cm")
...
downreg_results <- read.csv("brain-P5-downregulated-logFC-05.csv")
downreg_results$Description <- gsub("covid", "COVID", downreg_results$Description)
downreg_results$Description <- gsub("Tgf", "TGF", downreg_results$Description)
...
plot_df$Category <- "Immunologic"
plot_df$Category[c(11)] <- "Neurologic"
plot_df$Category[c( 7, 9, 13)] <- "Metabolic"
plot_df$Category[c(2, 4, 8, 12, 14, 15)] <- "Other"
plot_df$Category <- factor(plot_df$Category, 
                           levels = c("Immunologic", "Metabolic", "Neurologic", "Other"))
...
ggsave("brain-P5-downreg-pathways-barplot-top15-mono.pdf",
       height = 10.5,
       width = 15, 
       units="cm")
